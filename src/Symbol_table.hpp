...
inline std::pair<int, symbol_table_t::iterator> find_iter(const std::string &symbol)
{
...
// 插入符号定义
inline void insert_symbol(const std::string &symbol, symbol_type type, int value)
{
  int symtid;
...
inline int exist_symbol(const std::string &symbol)
{
  int symtid;
  symbol_table_t::iterator it;
  std::tie(symtid, it) = find_iter(symbol);
...
inline std::pair<std::string, std::shared_ptr<const symbol_value>> query_symbol(const std::string &symbol)
{
  int symtid;
  symbol_table_t::iterator it;
  std::tie(symtid, it) = find_iter(symbol);
